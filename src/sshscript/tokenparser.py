...
def convert(code):
    ...
    def warn_legacy_twodollars():
        nonlocal legacy_twodollars_warned
        if legacy_twodollars_warned:
            return
        legacy_twodollars_warned = True
        warnings.warn(
    ...
    while i < len(tokens):
        ...
        ## tokenize.NL Stands for "non-significant newlines."
            ...
            if offset > 0:
                ...
                ## mostly the next line, such as in multi-line statement, 
                ## but we still need to add space to align the column of next token
                chunk.append(' ')
            endCol = token.end[1]        
        ...
        #### two dollar ####
            ...
            warn_legacy_twodollars()
            ## Legacy $$ forces shell mode through the unified execution path.
            chunk.append('exec_command(')
            endCol = nexttoken.end[1]
            i,endCol = convertFstring(tokens,i+2,endCol,chunk)
            #chunk.append(',0)')
            chunk.append(', _legacy_twodollars=True)')
        elif is_dollar(token) and\
            nexttoken and\
            is_dollar(nexttoken) and\
            nextnexttoken and\
            ...
            warn_legacy_twodollars()
            suffix = ', _legacy_twodollars=True'
        ...
        ## this condition must after the above condition which searching for $$(..)
            ...
            #chunk.append(' """,1)')
            chunk.append(' """, _legacy_twodollars=True)')
        #### unified one-dollar execution ####
        elif is_dollar(token) and\
            nexttoken and\
            nexttoken.type == tokenize.OP and nexttoken.string=='(':
            ## $(hostname), $('hostname'), $(f'hostname') => exec_command(...)
            ## $(chr(3)) <= send Ctrl+C
            suffix = ''
            ## add space
            endCol = nexttoken.end[1]
            ...
            nexttoken and\
            is_fstring(nexttoken):
            ## $f'hostname' => exec_command(f'hostname')
            chunk.append('exec_command(')
            i,endCol = convertFstring(tokens,i+1,endCol,chunk)
            chunk.append(')')
        elif is_dollar(token) and\
