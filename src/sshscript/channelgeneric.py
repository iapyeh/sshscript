...
class GenericChannel(object):
    ...
    def exitcode(self)->int:
        ...
        if self.hijacked:
            ...
            logger.warning('actual exitcode is not available in current block, eg. in $.enter')
            return -1
    ...
    def expect(self,rawpat,timeout=None,stdout=True,stderr=True,silent=False):
        ...
        def found(m):
            ...
            if isinstance(rawpat,dict):
                if ret[0] is None:
                    ...
                    ret.append(m)
                ## not send here, since this routine might be called
        ...
        def listener(items):
            ...
            for pat in regularPats[:]:
                m = pat.search(''.join(items))
                if m :
                    found(m)

        _stdout_with_listener = None
        _stderr_with_listener = None
        def set_listener()->bool:
            nonlocal _stdout_with_listener
            nonlocal _stderr_with_listener
            if stdout:
                _stdout_with_listener = self._stdout
                _stdout_with_listener.push_listener(listener)
            if stderr:
                _stderr_with_listener = self._stderr
    ...
    def input(self,text):
        ...
        if self.hijacked:
            if self.prompt:
                ...
                ## such as when inputing password
                ## release the lock after 1 second's silience(no output)
    ...
    def get_exit_code(self,timeout=None):
        ...
        if self.prompt:
            ...
            self._stdout.set_callback(prompt_found_callback,'-@@-')
            self._stderr.set_callback(prompt_found_callback,'-@@-')
            self.send(f'echo "_{sno}_$?_{sno}{sno}_-@@-"\n')

        ## block until the exit code is retrieved and the executing_lock is released
        while not complete:
            time.sleep(0.01)
        ...
        if self.prompt:
            ...
            def prompt_found_callback():
                ...
                if m:
                    ...
                    logger.warning(f'can not find exitcode in {[str(self._stdout)]}')
                self.executing_lock.release()
    ...
    def reset_buffer(self,reason=None):
        ...
        #print(f'{self.prefixOfLog}reset_buffer, reason={reason}{"@~" * 50}')
        with self._lock:
